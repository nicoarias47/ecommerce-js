...
const showProduct = () => {
  for (let i = 0; i < array[i].length; i++) {
    array.forEach((el) => {
      if (el[i].precio > 0) {
        template.querySelector("#cTitle").textContent = el[i].name;
        template.querySelector("#cText").textContent = el[i].precio;
        template.querySelector("#card-img").setAttribute("src", el[i].img);
        template.querySelector("#btn-compra").dataset.id = el[i].id;
